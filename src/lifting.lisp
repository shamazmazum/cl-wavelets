...
(def-lifting-scheme (cdf-6-2 :cdf-6-2 :forward)
  ...
  (decf odd (truncate (ash (+ even even+1) 2) 3))
  (decf even (ash (- odd (* odd-1 5)) -5))
  (progn
    (incf odd (ash even 2))
...
(def-lifting-scheme (inv-cdf-6-2 :cdf-6-2 :inverse)
  (progn
    (incf odd even)
    (decf even (ash (* odd 3) -2))
    (decf odd (ash even 2)))
  (incf even (ash (- odd (* odd-1 5)) -5))
  (incf odd (truncate (ash (+ even even+1) 2) 3))
  (incf even (ash (* (+ odd odd-1) 9) -4))
  (incf odd (truncate (+ even even+1) 6)))
...
(def-lifting-scheme (cdf-3-1 :cdf-3-1 :forward)
  ...
  (progn
    ...
    (incf even (truncate odd 3))
    (decf odd even)))

(def-lifting-scheme (inv-cdf-3-1 :cdf-3-1 :inverse)
  (progn
    (incf odd even)
    (decf even (truncate odd 3))
    (decf odd  (ash (* 3 even) -1))
    (decf even (truncate (* 2 odd) 9)))
  (incf odd  (ash (+ (* 9 even)
                     (* 3 even+1))
                  -3))
  (incf even (truncate odd-1 3)))
